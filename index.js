...
class Shell {
    ...
    static async run(command = 'ls', after = (errs, output) => { }) {
        return await new Promise((resolve, _) => {
            exec(command, (errors, output) => {
                after(errors, output);
                resolve(output);
            });
        });
    }
    static async runSilently(command = 'ls', after = (errs, output) => { }) {
        return await new Promise((resolve, _) => {
            const finalCommand = command.includes('&')
                ? command.split('&')[0] + '> /dev/null ' + "&" + command.split('&')[1]
                : command + ' > /dev/null';
            exec(finalCommand, (errors, output) => {
                after(errors, output);
                resolve(output);
            });
        });
    ...
    async run(M, callable) {
        ...
        for (const i in requiredPatches) {
            ...
            for (const j in outputIpas) {
                const ipaName = outputIpas[j].split('.')[0];
                const patchName = requiredPatches[i];
                await callable(ipaName, patchName);
                patched++;
                const isComplete = patched === outputIpas.length;
                // @ts-ignore
...
const main = async () => {
    ...
    const IPAS = await M.get('ls Ipas');
    const IPA_NAME = IPAS[0].split('.')[0];
